...
$$Y_{ijk}=Cultivar_{ijk}+\beta_{1} Brix_{ijk}+\beta_{2}Titro_{ijk}+\beta_{3}Firmness_{ijk}+Harvest_{i}+Block_{j}+\epsilon_{ijk}$$

, where $Block_{ij}$~$N(0,\sigma_{b})$ is the random effect of the $j^{th}$ block (not nested in the harvest).

```{r}
lmm1 = lmer(Score ~ Entry + Brix + CorrectedTitro + Firmness.kg + +(1 | HarvestPeriod)+(1 | HarvestPeriod:Block), data = df.merge.bot2)
AIC(lmm1)
plot(lmm1, type=c("p","smooth"), col.line=1)
lattice::qqmath(lmm1)
```
